...
local CONMEMAYCAMP3 = Tabs.Player:AddSection("Magma Camp")
local awta = Tabs.Player:AddSlider("Slider", {
    Title = "Magma Multiple",
    Description = "Change this for adjusting Magma per hit",
    Default = 5,
    Min = 1,
    Max = 10,
    Rounding = 0,
    Callback = function(magamwt)
            magmatimet = tonumber(magamwt)
        end
    })

local a2wta = Tabs.Player:AddSlider("Slider", {
    ...
    Max = 100,
    Rounding = 0,
    Callback = function(distancecapmag)
            magdis = tonumber(distancecapmag)
        end
    })


local magmamgggg = Tabs.Player:AddToggle("magmamggggtoggle", {Title = "Camp Magma", Default = false })
...
pcall(function()
for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
if v.Name == SelectedKillPlayer and _G.CampMagma == true and v.Humanoid.Health > 0 then
    pcall(function()
repeat task.wait()
local args = {
...
until v.Humanoid.Health == 0 or not _G.CampMagma or c.Name ~= SelectedKillPlayer
end)
end
end
end)
...
if c.Name == SelectedKillPlayer and _G.CampMagma == true then
repeat task.wait()
if c.Humanoid.Health > 0 then
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = c.HumanoidRootPart.CFrame * CFrame.new(0,magdis,0)
end
until c.Humanoid.Health == 0 or not _G.CampMagma or c.Name ~= SelectedKillPlayer
end
end
end)
end
...
spawn(function() -- autofarm velocity
    while wait(0) do
        pcall(function()
            if _G.CampMagma or _G.autocannonslow or _G.autocannonplr or getgenv().bgtyhn or getgenv().fishing or getgenv().tret or getgenv().e or getgenv().c or getgenv().a or getgenv().t or getgenv().emladepnwwhat or getgenv().emladepnhat then
            ...
            elseif _G.CampMagma == false or _G.autocannonslow == false or _G.autocannonplr == false or getgenv().bgtyhn == false or getgenv().fishing == false or getgenv().tret == false or  getgenv().e == false or getgenv().c == false or getgenv().a == false or getgenv().t == false or getgenv().emladepnwwhat == false or getgenv().emladepnhat == false then
